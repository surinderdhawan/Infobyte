...
// Icon components for technologies
const TechIcon = ({ name }: { name: string }) => {
  ...
  if (iconClass) {
    return <i className={`${iconClass} colored text-4xl`}></i>
  }
...
const categories = ['All', 'Frontend', 'Backend', 'Cloud', 'Mobile', 'Database', 'DevOps', 'AI/ML']

export default function Technologies() {
  return (
    <section id="technologies" className="py-20 px-4 sm:px-6 lg:px-8 relative bg-gradient-to-b from-transparent to-primary-900/10">
      <div className="container mx-auto">
        ...
        >
          <h2 className="text-4xl md:text-5xl font-bold mb-4">
            <span className="gradient-text">Technologies We Work With</span>
          </h2>
          <p className="text-xl text-gray-400 max-w-2xl mx-auto">
            Cutting-edge technologies and frameworks to build modern, scalable solutions
          </p>
        </motion.div>
        ...
        <div className="flex flex-wrap justify-center gap-3 mb-12">
          {categories.map((category) => (
            <motion.button
              ...
              whileTap={{ scale: 0.95 }}
              className="px-6 py-2 glass rounded-full text-gray-300 hover:text-white hover:bg-primary-500/20 transition-all"
            >
              {category}
            </motion.button>
          ))}
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-6 gap-4">
          {technologies.map((tech, index) => (
            <motion.div
              ...
              className="glass rounded-lg p-4 text-center hover:glow transition-all cursor-pointer group"
            >
              <div className="flex justify-center mb-2">
                <TechIcon name={tech.name} />
              </div>
              <div className="text-lg font-bold gradient-text mb-1">{tech.name}</div>
